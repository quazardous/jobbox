...
pub const VERBS: &[Verb] = &[
    ...
    Verb { name: "queue", notes: "", summary: "hand work over before it starts, under a cap",
        tags: &["create"], effect: "creates pending work",
        flags: &[("--expect", "how long it should take, so it is not called out: 4h")] },
    Verb { name: "expect", notes: "", summary: "say how long a job should take, so it is not called out",
        tags: &["configure"],
        effect: "records against one job how long it was said to need; stops nothing and starts nothing",
        flags: NOTHING },
    ...
    Verb { name: "hook", notes: "", summary: "answer an agent CLI; `init` declares this one",
        ...
        flags: &[("--list", "name every client this binary can answer, and check its declared hook"),
                 ("--json", "as JSON"),
                 ("--no-endings", "when a turn ends, name lines left running, and leave endings to `jbx wait`")] },
